...
(function () {
  ...
  const ROOT = (function resolveBase() {
    if (window.OUR_LIBRARY_BASE_PATH) {
      let base = window.OUR_LIBRARY_BASE_PATH;
  ...
  const resolvePath = (target) => {
    ...
    try {
      return new URL(trimmed, ROOT).href;
    } catch (error) {
      console.warn('[web-shim] resolvePath fallback', { target, error });
      return ROOT + trimmed;
    }
  };

  const SQL_BASE = resolvePath('Assets/sql.js/');
  const SQL_JS = SQL_BASE + 'sql-wasm.js';
  ...
  async function cfg() {
    ...
    const configPath = resolvePath('Config/ourlibrary_google_config.json');
    const r = await window.__nativeFetch(configPath, { cache:'no-store' });
    if (!r.ok) throw new Error(`Config 404 ${configPath}`);
    S.cfg = await r.json();
    return S.cfg;
  }
  const localDbUrlFromConfig = (c)=> {
    if (!c) return null;
    if (c.database_url) return resolvePath(c.database_url);
    if (c.database_filename) return resolvePath(`Data/Databases/${encodeURIComponent(c.database_filename)}`);
    return null;
  };
  ...
  async function loadSQL(){ if(S.SQL) return S.SQL; await loadScript(SQL_JS);
    ...
    S.SQL = await window.initSqlJs({ locateFile:(f)=>SQL_BASE+f }); log('sql.js ready'); return S.SQL; }
